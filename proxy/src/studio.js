...
export class StudioState {
  ...
  async oauthStart(sapix = null) {
    ...
    auth.search = new URLSearchParams({ client_id: this.env.GOOGLE_CLIENT_ID, redirect_uri: `${this.env.STUDIO_ORIGIN}/api/studio/google/callback`, response_type: 'code', scope: sapix ? 'openid email' : 'openid email https://www.googleapis.com/auth/drive', ...(sapix ? (this.sapix.ownerEmail() !== this.env.STUDIO_OWNER_EMAIL ? { prompt: 'select_account' } : {}) : { access_type: 'offline', prompt: 'consent' }), state, code_challenge: await digest(verifier), code_challenge_method: 'S256', login_hint: sapix ? this.sapix.ownerEmail() : this.env.STUDIO_OWNER_EMAIL }).toString();
    const value = await seal({ state, verifier, expires: Date.now() + 600000, ...(sapix ? { sapix: { ...sapix, email: this.sapix.ownerEmail() } } : {}) }, this.env.STUDIO_SECRET, 'oauth');
  ...
  async oauthCallback(request, url) {
    ...
    if (value.sapix && (!value.sapix.email || value.sapix.email !== this.sapix.ownerEmail())) fail(403, 'forbidden');
