...
const CLIENT_SCRIPT = String.raw`(function studioClient() {
  ...
  async function downloadPdf(event) {
    ...
    try {
      ...
      // Embedded browsers may not support blob downloads. Keep a native HTTP
      // attachment link as the explicit fallback after verifying the response.
      const save = $('save-downloaded-pdf'); save.href = '/api/studio/chat/pdf/' + encodeURIComponent(fileId(file)) + '?modifiedTime=' + encodeURIComponent(file.modifiedTime); save.download = filename; save.hidden = false;
      ...
      catch { $('pdf-download-status').textContent = '取得が完了しました。「PDFを直接保存」を押して保存してください。'; }
      finally { link.remove(); }
    } catch (error) {
