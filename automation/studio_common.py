...
LESSON_STAGE_INSTRUCTIONS = (
    ...
    " The renderer draws diagram text only from actual label primitives and their text fields. An ID such as title,"
    " row or count, diagram.description, cue text, a visible line or an arrow never creates a text label automatically."
    " Choose kind separately for each primitive. Inspect actual label data and each relevant cue's visibleIds before"
    " claiming that required annotations are implemented. In an independent review, identify missing text using actual"
    " primitive IDs, their current kinds and the needed text whenever possible; never trust a self-verification claim"
    " instead of inspecting the array. Keep missing required annotations unresolved until the actual data is repaired."
    " This is one assigned problem within a complete-PDF pipeline. Separate preceding stages inventory every problem"
    " and reconcile official-solution pages; separate problem calls are combined and checked for complete coverage."
    " Requirements to include every problem apply to that final collection. In this call, verify every subquestion of"
    " the assigned problem and source connections needed for it. Other problems visible on the same image retain their"
    " own assignments; do not recursively demand all unrelated pages merely because they are absent from this call."
    " Do not claim to have personally checked images or inventory evidence that was not supplied. When inventory or"
    " page-management evidence is supplied, compare it with the actual images rather than blindly trusting it."
    " Any concrete contradiction, missing source for the assigned problem, or unresolved relevant page connection"
    " remains an unresolved issue; the division into stages is never evidence that such an issue was resolved."
