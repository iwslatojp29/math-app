...
import { formatAnswerText } from './renderer/answer-format.mjs';
...
function staticProblem(problem) {
  ...
  const answers = problem.subquestions.map(sub => '<li><strong>' + escapeHtml(sub.label) + '</strong> ' + escapeHtml(formatAnswerText(sub.answerText, sub.unit)) + (sub.answerFormulaId ? formulaList(problem, [sub.answerFormulaId]) : '') + '</li>').join('');
...
export async function renderLesson(lesson, { assets = {}, assetBaseDirectory = process.cwd(), maxBytes = MAX_HTML_BYTES } = {}) {
  ...
  const answerRows = lesson.problems.flatMap(problem => problem.subquestions.map(sub => '<tr><td>' + escapeHtml(lesson.sections.find(section => section.id === problem.sectionId).title) + '</td><td>' + escapeHtml(problem.number) + '</td><td>' + escapeHtml(sub.label) + '</td><td>' + escapeHtml(formatAnswerText(sub.answerText, sub.unit)) + '</td></tr>')).join('');
